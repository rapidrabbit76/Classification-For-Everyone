...
class MNASNetModel(pl.LightningModule):
...
def train():
    ...
    # Model
    mnasneta1 = MNASNetModel(config=config)
    ...
    wandb_logger.watch(mnasneta1, log='all', log_freq=100)
    ...
    # Train
    trainer.fit(mnasneta1, datamodule=dm)
    trainer.test(mnasneta1, datamodule=dm)

    # Finish
    wandb_logger.experiment.unwatch(mnasneta1)
